...
The function \verb!logcheby! takes as input a matrix \verb!B! and vector \verb!f!, and returns the solution to the Logarithmic Chebyshev Approximation problem using \verb!sqlp!.
